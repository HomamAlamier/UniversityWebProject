                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        <?php
                            ...
                            || $perms->PERM_EDIT_ADMIN == true 
                            || $perms->PERM_DEL_ADMIN == true )
                                foreach ($array_admins as $admin) {
                                    echo "<tr><td>$admin->username</td>";
                                    echo "<td>$admin->email</td>";
                                    echo "<td>$admin->password</td>";
                                    echo "<td>$admin->address</td>";
                                    echo "<td>$admin->permissions</td>";
                                    echo "<td>$admin->added_by</td>";
                                    if ($perms->PERM_EDIT_ADMIN == true || $perms->PERM_ALL == true)
                                        echo '<td><button class="btn btn-primary"><i class="fa fa-edit fa-lg"></i></button></td>';
                                    if ($perms->PERM_DEL_ADMIN == true || $perms->PERM_ALL == true)
                                        echo '<td><button class="btn btn-danger" onclick="delete_admin(' . $admin->id . ')"><i class="fa fa-remove fa-lg"></i></button></td>';
                                    echo '</tr>';
                                }
                            else {
                                echo "<center><h3>You don't have the permissions to manage admins !</p></center>";
                            }
                        ?>
                    </tbody>
                </table>
...
</html>    <script src="{{ asset('js/shared.js') }}"></script>